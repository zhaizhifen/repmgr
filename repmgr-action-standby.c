...
do_standby_follow_internal(PGconn *primary_conn, PGconn *follow_target_conn, t_node_info *follow_target_node_record, PQExpBufferData *output, int general_error_code, int *error_code)
{
	...
	if (PQserverVersion(primary_conn) >= 120000)
	{
		if (configure_replication(&local_node_record, &recovery_conninfo) == false)
		{
 ...
 *
 * From Pg12, settings previously written to "recovery.conf" (which had
 * a predictable, stable location) will now form part of the main
 * configuration settings.
 *
 * This means that in theory, the settings could be placed anywhere where
 * normal PostgreSQL configuration lives, e.g. in nested included files
 * located far outside the data directory.
 *
 * To keep things as simple as possible, by default we'll do what
 * "pg_basebackup --write-recovery-conf" does, i.e. append the settings to
 * "postgresql.auto.conf", which remains in a predicatble location and is
 * guaranteed to be evaluated last.
 *
 * For the initial implementation, we'll stick to appending to the file,
 * which does however mean it could end up littered with redundant repmgr
 * configuration sections (this is what pg_basebackup does too, see function
 * WriteRecoveryConf() in pg_basebackup.c).
 *
 * As an alternative, we'll enable the user to specify an alternative file
 * with "replication_config", which will be *overwritten* by repmgr when required.
 * The onus will be on the user to ensure the file is correctly included in the
 * main configuration and is not superseded by other settings.
 *
 * TODO: We should also add a "repmgr standby check" command to verify the replication
 * configuration has been included and the settings match those expected.
 */
...
configure_replication(t_node_info *node_record, t_conninfo_param_list *recovery_conninfo)
{
	...
	FILE	   *file;
	mode_t		um;
	bool	    using_postgresql_auto_conf;

	/*
	 * if "replication_config" not specified, that means we're appending
	 * to "postgresql.auto.conf"
	 */
	using_postgresql_auto_conf = get_node_replication_configuration_file(replication_config_file_path);

	/* create recovery configuration in buffer */
	initPQExpBuffer(&replication_config_buf);

	// TODO: append date
	appendPQExpBuffer(&replication_config_buf,
					  "# Configuration generated by repmgr %s\n\n",
					  REPMGR_VERSION);
	...
	if (using_postgresql_auto_conf)
	{
		log_debug("configure_replication(): appending to \"%s\"...",
				  DEFAULT_REPLICATION_CONFIG_FILE);

		file = fopen(replication_config_file_path, "a");
		if (file == NULL)
		{
			log_error(_("unable to open \"%s\""),
		...
		if (fwrite(replication_config_buf.data, replication_config_buf.len, 1, file) != 1)
		{
			log_error(_("unable to write to \"%s\""),
					  replication_config_file_path);
			log_detail("%s", strerror(errno));
